...
from src.database.db import enroll_student_to_subject, get_client
    ...
    if st.button('Enroll now', type='primary', use_container_width=True):
        if join_code:
            ...
            if res.data:
                ...
                check = client.table('subject_students').select('*').eq('subject_id', subject['subject_id']).eq('student_id', student_id).execute()
                if check.data:
                ...
                st.error('Subject code not found. Please verify with your teacher.')
        else:
